...
passport_1.default.use(new passport_google_oauth20_1.Strategy({
    ...
    try {
        ...
        let doesUserExist = yield user_model_1.User.findOne({ email });
        if (!doesUserExist) {
            return done(null, false, { message: "User does not Exist" });
        }
        if (doesUserExist &&
            (doesUserExist.isActive === user_interface_1.isActive.BLOCKED ||
        ...
        if (!doesUserExist) {
            doesUserExist = yield user_model_1.User.create({
                email,
...
passport_1.default.use(new passport_local_1.Strategy({
    ...
    try {
        const doesUserExist = yield user_model_1.User.findOne({ email });
        if (!doesUserExist) {
            return done(null, false, { message: "User does not Exist" });
        }
        ...
        const isGoogleAuthenticated = doesUserExist.auths.some((providerObjects) => providerObjects.provider === "google");
        ...
        const isPasswordMatched = yield bcryptjs_1.default.compare(password, doesUserExist.password);
